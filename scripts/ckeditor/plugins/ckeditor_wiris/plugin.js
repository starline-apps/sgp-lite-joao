	...
	'init': function (editor) {
		...
		function checkElement() {
			try {
				...
				if (editor.elementMode == CKEDITOR.ELEMENT_MODE_INLINE || editor.isDivArea) {
				...
				if (element != newElement) {
					if (editor.elementMode == CKEDITOR.ELEMENT_MODE_INLINE || editor.isDivArea) {
						...
						$('#cke_'+editor.name).mouseover(function(){$('#'+editor.id+'_contents').children().focus()});
					}
					else {
		...
		if (_wrs_conf_editorEnabled) {
			...
			editor.addCommand('ckeditor_wiris_openFormulaEditor', {
				...
				'exec': function (editor) {
					var isFrame = editor.isDivArea ? false : editor.elementMode != CKEDITOR.ELEMENT_MODE_INLINE;
					wrs_int_openNewFormulaEditor(element, editor.langCode, isFrame);
				}
			});
		...
		if (_wrs_conf_CASEnabled) {
			...
			editor.addCommand('ckeditor_wiris_openCAS', {
				...
				'exec': function (editor) {
					wrs_int_openNewCAS(element, editor.elementMode != CKEDITOR.ELEMENT_MODE_INLINE);
				}
			});
